...
function validateAndFix(opts = {}) {
  ...
  let cleanOpts = (0, _lodash2.default)(opts);
  if (cleanOpts.plugins === void 0) cleanOpts.plugins = [];
  if (!Array.isArray(cleanOpts.plugins)) throw new Error("Expected options.svgo.plugins to be an array");

  if (cleanOpts.plugins.length === 0) {
    cleanOpts.plugins = [...essentialPlugins].map(p => ({
      [p]: true
    }));
  }
